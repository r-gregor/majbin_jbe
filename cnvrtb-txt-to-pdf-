...
if [ ! ${FJL_ext} == "txt" ]; then
    tms; echo -e "File must be *.txt file!\n"
    exit
else
...
if [ ! -e ${FJL} ]; then
    tms; echo -e "File [ ${FJL} ] NOT found!\n"
    exit
fi

# test if dest file already exists
if [ -e ${FJLPDF} ]; then
    tms; echo -e "File [ ${FJLPDF} ] already exists!\n"
    exit
fi
...
enscript -v -f "Iosevka8" -F "Iosevka8" --header='$n||Page $% of $=' -p ${FJLPS} ${FJL}
ps2pdf ${FJLPS} ${FJLPDF}
